    ...
    private void handleField(Object o, String key, JSONObject jobject) {
        try {
            ...
            if (field.getType().getName().contains("ArrayList")) {
                ...
                {
                    Object object = new JsonToObject().GetObject(jobject.getJSONObject(key), field.getType());
                    field.set(o, object);
                }
            }
            // }
    ...
    private void handleFieldAnnotated(Object o, Field field, String key, JSONObject jobject) throws JSONException, IllegalAccessException {
        ...
        if (map.Type().contentEquals(JPM.B)) {
            try {
                ...
                if (sVal.contentEquals("1") || sVal.contentEquals("T") || sVal.contentEquals("true")) {
            ...
            if (jarray != null && jarray.length() > 0) {
                for (int i = 0; i < jarray.length(); i++) {
                    Object ao = new JsonToObject().GetObject(jarray.getJSONObject(i), map.Klass());
            ...
            try {
                ...
                Typeface face = Typeface.createFromAsset(JParse.context.getAssets(), "fonts/" + fontpath);
                field.set(o, face);
            } catch (Exception ex) {
                ex.printStackTrace();
            }
